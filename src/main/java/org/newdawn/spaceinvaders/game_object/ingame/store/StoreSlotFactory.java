...
public class StoreSlotFactory {
    ...
    public void createPassiveSkillItemSlot(int skillId, PositionAngleSet positionAngleSet){
        StorePassiveItem passiveSkill = generatePassiveSkill(skillId);

        if (passiveSkill != null) {
            spawnToGameLoop(passiveSkill, positionAngleSet);
        }
    }
    ...
    private StorePassiveItem generatePassiveSkill(int skillId) {
        StorePassiveItem storePassiveItem = null;
        switch (skillId) {
            case PS_FIRE_SPEED:      storePassiveItem = new StorePassiveItem(PlayerPassiveSkillType.FIRE_SPEED); break;
            case PS_DAMAGE_UP:       storePassiveItem = new StorePassiveItem(PlayerPassiveSkillType.DAMAGE_UP); break;
            case PS_ADDITIONAL_ENGINE: storePassiveItem = new StorePassiveItem(PlayerPassiveSkillType.ADDITIONAL_ENGINE); break;
            case PS_REPAIR_KIT:      storePassiveItem = new StorePassiveItem(PlayerPassiveSkillType.REPAIR_KIT); break;
            default:
                System.err.println(skillId + "은 존재하지 않은 passiveSkill ID 입니다.");
                storePassiveItem = null;
                break;
        }
        return storePassiveItem;
    }
